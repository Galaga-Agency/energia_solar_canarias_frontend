...
const SortUserMenu = ({ onSortChange }) => {
  ...
  const options = [
    ...
    },
    {
      ...
      icon: <BsCalendar />,
      label: t("sortByRegistrationDate"),
    },
  ];
  ...
  return (
    <div className="relative w-fit z-0" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute z-10 mt-1 bg-slate-50 dark:bg-slate-700/50 border border-gray-300 dark:border-custom-light-gray rounded-lg shadow-lg min-w-[100%] whitespace-nowrap">
          {options.map((option) => (
            <button
              key={option.value}
