...
const NotificationsTab = () => {
  ...
  const handleMarkAsRead = (notificationId, plantId) => {
    window.location.href = `/dashboard/${userId}/plants/${plantId}`;
  ...
  return (
    <div className="p-8 text-white flex flex-col pb-24">
      <Texture />
      ...
      <div className="mt-6 space-y-12" ref={notificationsRef}>
        <div className="space-y-4">
          <h3 className="text-2xl text-custom-dark-blue dark:text-custom-yellow">
            {t("activeNotifications")}
          </h3>
          {activeNotifications.length === 0 ? (
            <p className="text-gray-500 text-center ">{t("youAreUpToDate")}</p>
          ) : (
            <AnimatePresence>
                  ...
                  archivedNotifications.map((notification) => (
                    ...
                    >
                      <NotificationCard
                        ...
                        read={notification.read}
                        plantName={notification.plantName}
                        undoArchive={() => handleOpenUndoModal(notification.id)}
