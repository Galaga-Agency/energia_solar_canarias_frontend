...
const GoodweSortMenu = ({ onSortChange }) => {
  ...
  return (
    <div className="relative w-fit z-30" ref={dropdownRef}>
      <button
        ...
        className="w-full flex items-center justify-between border-1 border-gray-300 dark:border-gray-600 text-custom-dark-blue dark:text-custom-yellow font-secondary px-4 py-2  bg-white/50 dark:bg-custom-dark-blue/50 rounded-lg shadow-sm focus:outline-none focus:ring focus:ring-custom-yellow focus:ring-opacity-50 transition duration-300"
      >
      ...
      {isOpen && (
        <div className="absolute z-10 mt-1 bg-white/50 backdrop-blur-sm dark:bg-custom-dark-blue border-1 border-gray-300 dark:border-custom-light-gray rounded-lg shadow-lg min-w-[100%] whitespace-nowrap">
          {options.map((option) => (
            <button
              key={option.value}
